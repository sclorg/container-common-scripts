    ...
    for version in versions:
        ...
        for distro in distros:
            ...
            if distro in docker_distros[version]:
                table += f"<details><summary>✓</summary>`{distro_names[distro][1] % (name + "-" + version.replace('.', ''))}`</details>"
        # end the table line
        table += '|\n'
    return table
...
def _replace_in_readme(table):
    try:
        with open("README.md", "r+") as readme:
            ...
            if subs == 0:
                print("The Table start and Table end tag not found, not modifying README.md", file=sys.stderr)
                exit(0)
            if subs > 1:
                print("More than one Table start and Table end tag found, not modifying README.md", file=sys.stderr)
                exit(0)
            readme.seek(0)
